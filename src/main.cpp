...
#if defined(NDEBUG)
# error "Bumbacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd674b9d6b33abef2ebbaf8b3d5650ea80a256a36f04cc3ae881935f664f6dadd");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bumbacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bumbacoin Signed Message:\n";
...
{
    // Bumbacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 10080 blocks, which will occur approximately every 1 week
    nSubsidy >>= (nHeight / 10080); 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 60 * 60; // 5 hours
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bumbacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bumbacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Bumbacoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd674b9d6b33abef2ebbaf8b3d5650ea80a256a36f04cc3ae881935f664f6dadd");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1399017219;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086079529;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Bumbacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BumbacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BumbacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BumbacoinMiner(CWallet *pwallet)
{
    printf("BumbacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bumbacoin-miner");
    ...
    try { loop {
        ...
        printf("Running BumbacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BumbacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BumbacoinMiner, pwallet));
}
